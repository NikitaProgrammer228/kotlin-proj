...
object MeasurementMath {
    ...
    fun calcCoordinationFactor(
        ...
        // ─── Коэффициент хаотичности (0..~0.6) ───
        // v7.2: Увеличено влияние общей амплитуды, уменьшено влияние хаотичности
        // (ближе к немецкому алгоритму, который больше учитывает амплитуду)
        // 0 = плавные монотонные движения
        // ~0.6 = хаотичные разнонаправленные движения
        val chaosCoef = (irregularityRatio * 0.15 + jitterRatio * 0.10 + discordance * 0.10)
            .coerceIn(0.0, 0.6)
        
        // ─── Итоговая формула КФ v7.2 ───
        // КФ = Сумма_амплитуд × (0.55 + хаотичность)
        // Базовый множитель выше → амплитуда влияет сильнее
        // При плавных движениях: множитель ≈ 0.55-0.65
        // При хаотичных движениях: множитель до 1.15
        val rawKF = sumAmplitudes * (0.55 + chaosCoef)
