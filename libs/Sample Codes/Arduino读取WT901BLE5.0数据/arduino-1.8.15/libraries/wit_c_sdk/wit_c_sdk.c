...
void WitSerialDataIn(uint8_t ucData)
{
    ...
    switch(s_uiProtoclo)
    {
        case WIT_PROTOCOL_BLE:
            if((s_ucWitDataBuff[0] != 0x55) && (s_ucWitDataBuff[1] != 0x61))
            {
                s_uiWitDataCnt--;
            ...
            {
                sReg[AX] = ((uint16_t)s_ucWitDataBuff[3] << 8) | (uint16_t)s_ucWitDataBuff[2];
                sReg[AY]  = ((uint16_t)s_ucWitDataBuff[5] << 8) | (uint16_t)s_ucWitDataBuff[4];
                sReg[AZ]  = ((uint16_t)s_ucWitDataBuff[7] << 8) | (uint16_t)s_ucWitDataBuff[6];
                sReg[GX] = ((uint16_t)s_ucWitDataBuff[9] << 8) | (uint16_t)s_ucWitDataBuff[8];
                sReg[GY] = ((uint16_t)s_ucWitDataBuff[11] << 8) | (uint16_t)s_ucWitDataBuff[10];
                sReg[GZ] = ((uint16_t)s_ucWitDataBuff[13] << 8) | (uint16_t)s_ucWitDataBuff[12];
	sReg[Roll] = ((uint16_t)s_ucWitDataBuff[15] << 8) | (uint16_t)s_ucWitDataBuff[14];
                sReg[Pitch] = ((uint16_t)s_ucWitDataBuff[17] << 8) | (uint16_t)s_ucWitDataBuff[16];
                sReg[Yaw] = ((uint16_t)s_ucWitDataBuff[19] << 8) | (uint16_t)s_ucWitDataBuff[18];
	p_WitRegUpdateCbFunc(AX, 3);
	p_WitRegUpdateCbFunc(GX, 3);
	p_WitRegUpdateCbFunc(Roll, 3);
                s_uiWitDataCnt = 0;
            }
        break;
